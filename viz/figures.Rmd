...
```{r Figure2, fig.width = 10, fig.height = 4}
...
```{r Figure3, fig.width = 10, fig.height = 6}

lang_plotter <- function(data, x_lab, y_lab = NULL) {
...
lang_plotter(mods$languages$mdt, "Melodic Discrimination", "Language") + 
lang_plotter(mods$languages$mpt, "Mistuning Perception") + 
lang_plotter(mods$languages$cabat, "Beat Alignment") +
...
```{r OLD-Figure4, fig.width = 10, fig.height = 6, eval=FALSE, include=FALSE}
...
lang_plotter <- function(data_x, x_lab, y_lab) {
  ggplot(data = confirm_plot_data %>%
    ...
    geom_boxplot(aes(y = reorder(language, {{data_x}}, median), x = {{data_x}},
                     fill = factor(Tone, levels = c("Non-tonal","Pitch-accented", "Tonal"))), outlier.shape=NA) +
...
```{r New-SI-Figure1, fig.width = 12, fig.height = 7}
...
```{r FigureStemp, fig.width=10, fig.height = 4}

plot <- fig3_data %>%
  ggplot() +
  geom_abline(intercept = 0, slope = 1, alpha = 1, size = .3) +
  geom_point(aes(x = exploratory, y = confirmatory, color = Tone),
             alpha = 0.8, size = 2) +
  facet_wrap(
    ~ factor(test, levels = c("mdt", "mpt", "cabat")), 
    labeller = labeller(.rows = c("mdt" = "Melodic Discrimination", "mpt" = "Mistuning Perception", "cabat" = "Beat Alignment"))
  ) +
...
```{r FigureS1a, fig.width = 7, fig.height = 8, include=FALSE}
...
format_language_label <- function(data, add_count = FALSE) {
  data <- data |> 
    mutate(level_clean = case_when(
      ...
      level == "Chinese/Cantonese/Yue" ~ "Yue",
      level == "Chinese/Other dialects" ~ "Other Chinese languages",
...
\clearpage
```{r FigureS1b, fig.width = 7, fig.height = 8, include=FALSE}
...
\clearpage
```{r FigureS1c, fig.width = 7, fig.height = 8, include=FALSE}
...
\newpage
```{r figureTest, fig.width = 21.2, fig.height = 15, out.extra='angle=90', out.width="100%", out.height="100%"}
...
meta_plotter_combined <- function(data) {
  ...
  ggplot(dat, aes(y = row, x = TE, xmin = lower, xmax = upper,
    ...
    annotate("text", x = melodic_effect + 0.55, xend = melodic_effect + 0.55, y = 34, yend = 34, label = r2(melodic_effect)) +
    annotate("text", x = pitch_effect + 0.55, xend = pitch_effect + 0.55, y = 11, yend = 11, label = r2(pitch_effect)) +
    annotate("text", x = rhythm_effect + 0.6, xend = rhythm_effect + 0.6, y = 1, yend = 1, label = r2(rhythm_effect)) +
    geom_vline(xintercept = 0) +
    geom_point() +
    geom_errorbar(size = 0.6) +
    ...
    scale_shape_manual(values = c(19,5)) +
    scale_size_manual(values = c(2, 5)) +
    coord_cartesian(ylim = c(1.75, 45)) +
    theme_classic() + 
    theme(
      legend.position = "none",
      axis.text.y = element_blank(),
...
meta_fig_assembler2 <- function(meta_data1, meta_data2, meta_data3, filename,
  ...
  forest_plot_creator(left_side_data = select(combined, 1:5) |> mutate(Study = ifelse(Study == "Overall", " ",Study)),
                      ...
                      left_offset = left_offset,
                      right_offset = right_offset,
                      file_path = here::here("viz", "temp", filename))
}
...
pitch.gen.random$clean <- pitch.gen.random$data |>
  ...
  mutate(
    Study = case_when(
      `Non-tonal` == "11 (Portuguese, 1)" ~ "Pfordresher & Brown (2009), Study 2",
      `Non-tonal` == "17 (No)" && Study == "Giuliano et al. (2011)" ~ "Giuliano et al. (2011), Study 1",
  ...
  mutate(
    ...
    Tonal = case_when(
      ...
      Study == "Pfordresher \\& Brown (2009)" ~ "6 Vietnamese; 4 Mandarin; 2 Cantonese",
      Study == "Pfordresher & Brown (2009), Study 2" ~ "11 Mandarin",
      Study == "Wong et al. (2012), Study 1" ~ "408 Cantonese",
      Study == "Wong et al. (2012), Study 2" ~ "408 Cantonese",
      Study == "Zheng \\& Samuel (2018)" ~ "24 Mandarin",
    ...
    `Non-tonal` = case_when(
      ...
      Study == "Wong et al. (2012), Study 2" ~ "154 English/French",
...
\clearpage
```{r meta-analysis, fig.height=4, fig.width=18, out.extra='angle=90'}


meta_fig_assembler2(melodic.gen.random, pitch.gen.random, rhythm.gen.random,
                    "mega_meta-plot.pdf", top_offset = 0.072, bottom_offset = -0.02, right_offset = -0.015)
...
```{r FigureS2, fig.height=4, fig.width=12}

permutation_plot <- function(data, title, fill_col) {
...
permutation_plot(permutation_tests$T_NT$mdt, "Melodic discrimination", "yellow") + 
  permutation_plot(permutation_tests$T_NT$mpt, "Mistuning discrimination", "yellow") +
  permutation_plot(permutation_tests$T_NT$cabat, "Beat alignment", "yellow") + 
  plot_annotation(title = "Tonal vs Non-tonal speakers", theme = theme(plot.title = element_text(hjust = 0.5, size = 18)))


```


```{r FigureS3, fig.height=4, fig.width=12}

permutation_plot(permutation_tests$PA_NT$mdt, "Melodic discrimination", "blue") + 
  permutation_plot(permutation_tests$PA_NT$mpt, "Mistuning discrimination", "blue") + 
  permutation_plot(permutation_tests$PA_NT$cabat, "Beat alignment", "blue") + 
  plot_annotation(title = "Pitch-accent vs Non-tonal speakers", theme = theme(plot.title = element_text(hjust = 0.5, size = 18)))
